...
# ================= AUTOMOD + STICKY MESSAGE (AUTOMATA SZÖVEG) =================
...
async def on_message(msg):
    ...
    # AUTO SZÖVEG (STICKY) - TÖRLI ÉS ÚJRAÍRJA
...
async def sticky_set(i: discord.Interaction, szöveg: str):
    data = load_json(STICKY_FILE)
    data[str(i.channel.id)] = szöveg
    save_json(STICKY_FILE, data)
    await i.response.send_message(f"✅ Auto szöveg beállítva ebben a csatornában!", ephemeral=True)

@bot.tree.command(name="auto_szöveg_törlés")
@app_commands.check(mod_check)
async def sticky_del(i: discord.Interaction):
    data = load_json(STICKY_FILE)
    cid = str(i.channel.id)
    if cid in data:
        del data[cid]
        save_json(STICKY_FILE, data)
        await i.response.send_message("🗑 Auto szöveg kikapcsolva.", ephemeral=True)
    else: await i.response.send_message("❌ Nincs beállítva szöveg.", ephemeral=True)
