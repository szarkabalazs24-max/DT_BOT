...
TOKEN = os.getenv("DISCORD_TOKEN") # Vagy írd be: "A_TE_TOKENED"
...
class TicketDropdown(discord.ui.Select):
    def __init__(self):
        ...
        super().__init__(placeholder="Válassz kategóriát...", custom_id="ticket_select_main")
    ...
    async def callback(self, interaction: discord.Interaction):
        ...
        if not category or not role:
            return await interaction.response.send_message("❌ Hiba: Ez a kategória vagy rang nincs beállítva!", ephemeral=True)
        ...
        e = Embed(title=f"🎫 {valasztott.upper()}", color=Color.blue(), timestamp=datetime.datetime.utcnow())
        ...
        e.description = f"Szia {interaction.user.mention}! A(z) **{role.name}** csapat hamarosan válaszol."
        e.set_footer(text="✨ SERVICE HUN ✨")

        await ticket_ch.send(content=f"{role.mention} | Új ticket érkezett!", embed=e)
        await interaction.response.send_message(f"✅ Ticket létrehozva: {ticket_ch.mention}", ephemeral=True)
...
class GiveawayModal(ui.Modal, title='🎁 Nyereményjáték Beállítása'):
    ...
    async def on_submit(self, interaction: discord.Interaction):
        ...
        # LEZÁRÁS (A beküldött kép alapján)
        conn = sqlite3.connect('giveaway.db'); c = conn.cursor()
...
async def on_message(msg):
    ...
    if re.search(LINK_REGEX, txt): indok = "Tiltott link küldése"
        ...
        if len(user_messages[uid]) >= 5: indok = "Spammelés"
...
@bot.tree.command(name="figyelmeztetés", description="⚠️ Warn kiosztása")
@app_commands.check(tester_and_up)
async def warn(i, tag: discord.Member, indok: str):
    if is_target_mod(tag): return await i.response.send_message("🛑 Moderátort nem warnolhatsz!", ephemeral=True)
    ...
    e = make_embed("⚠️ Figyelmeztetés", f"👤 {tag.mention}\n📄 **Indok:** {indok}\n📊 **Warnok:** {len(data[uid])}\n👮 **Intézkedett:** {i.user.mention}", Color.orange())
    await i.response.send_message(embed=e)

@bot.tree.command(name="figyelmeztetés_törlés", description="🗑️ Warnok levonása")
@app_commands.check(mod_and_up)
async def warn_del(i, tag: discord.Member, mennyiseg: int):
    data = load_json(WARN_FILE); uid = str(tag.id); warns = data.get(uid, [])
    törölt = min(mennyiseg, len(warns))
    for _ in range(törölt): warns.pop()
    data[uid] = warns; save_json(WARN_FILE, data)
    e = make_embed("🧹 Warn Törlés", f"👤 {tag.mention}\n📉 **Törölve:** {törölt} db\n⚠️ **Maradt:** {len(warns)} db", Color.green())
    await i.response.send_message(embed=e)

# ================== 🎫 TICKET KEZELÉS 🎫 ==================

@bot.tree.command(name="ticket_beállítás", description="⚙️ Téma, kategória és rang összekötése")
@app_commands.check(mod_and_up)
...
async def t_setup(itn, tema: str, kategoria: discord.CategoryChannel, rang: discord.Role):
    config = load_json(TICKET_CONFIG_FILE)
    config[tema] = {"category_id": kategoria.id, "role_id": rang.id}
    save_json(TICKET_CONFIG_FILE, config)
    await itn.response.send_message(f"✅ {tema} beállítva: {kategoria.name} | {rang.mention}", ephemeral=True)

@bot.tree.command(name="ticketpanel", description="📨 Panel kiküldése")
@app_commands.check(mod_and_up)
async def tpanel(itn):
    await itn.response.send_message(embed=make_embed("📩 SEGÍTSÉG", "Válassz kategóriát!", Color.blue()), view=TicketView())

@bot.tree.command(name="hozzáad", description="➕ Tag hozzáadása tickethez")
async def t_add(itn, tag: discord.Member):
    if "ticket-" in itn.channel.name:
        ...
        await itn.response.send_message(f"✅ {tag.mention} hozzáadva!")

# ================== 🎉 EGYÉB FUNKCIÓK 🎉 ==================

@bot.tree.command(name="nyeremenyjatek")
@app_commands.check(mod_and_up)
async def g_start(itn): await itn.response.send_modal(GiveawayModal())

@bot.tree.command(name="videó")
@app_commands.check(video_check)
async def video_cmd(i, szoveg: str, video: discord.Attachment):
    data = load_json(VIDEO_FILE); data["count"] = data.get("count", 147) + 1; save_json(VIDEO_FILE, data)
    await i.response.send_message(content=f"**{data['count']}. Trade bizonyíték**\n{szoveg}", file=await video.to_file())
