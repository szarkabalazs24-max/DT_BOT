...
# A TE SAJÁT ID-D (FIXEN BELEÍRVA)
...
def mod_and_up(i: discord.Interaction):
    user_role_ids = [role.id for i_role in i.user.roles for role in [i_role] if True] # Eredeti struktúra megtartása
    user_role_ids = [role.id for role in i.user.roles]
    ...
    async def toggle_join(self, interaction: discord.Interaction, button: ui.Button):
        ...
        for i, field in enumerate(embed.fields):
            if "Jelentkezők" in field.name:
                embed.set_field_at(i, name="👤 Jelentkezők", value=f"**{count[0]}** fő", inline=False)
                break
...
async def on_member_join(member):
    ar = load_json(AUTO_ROLE_FILE); role_id = ar.get("role_id")
    if role_id:
        role = member.guild.get_role(int(role_id))
        if role: await member.add_roles(role)
    data = load_json(WELCOME_FILE); ch_id = data.get("channel_id")
    if ch_id:
...
async def on_member_remove(member):
    data = load_json(LEAVE_FILE); ch_id = data.get("channel_id")
    if ch_id:
        ch = member.guild.get_channel(int(ch_id))
...
async def reroll(i: discord.Interaction, uzenet_id: str):
    conn = sqlite3.connect(DB_FILE); c = conn.cursor()
    c.execute("SELECT user_id FROM parti
