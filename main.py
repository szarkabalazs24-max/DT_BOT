...
class GiveawayModal(ui.Modal, title='Nyereményjáték Beállítása'):
    ...
    async def process_winners(self, interaction, prize, count, msg_id, view):
        conn = sqlite3.connect('giveaway.db'); c = conn.cursor()
        c.execute("SELECT user_id FROM participants WHERE msg_id = ?", (str(msg_id),)); users = [row for row in c.fetchall()]; conn.close()
        for item in view.children: item.disabled = True
        try:
            msg = await interaction.channel.fetch_message(msg_id); old_embed = msg.embeds
            old_embed.title = "🔒 NYEREMÉNYJÁTÉK LEZÁRULT"; old_embed.color = discord.Color.dark_grey()
        ...
        else: await interaction.channel.send(f"😢 A(z) **{prize}** sorsolása sikertelen (nincs jelentkező).")
...
async def mute(i, tag: discord.Member, időtartam: str, indok: str):
    ...
    raw = időtartam.lower(); sec = int(raw.replace('m','h','d'))*60 if 'm' in raw else int(raw)*60
