...
TESTER_MOD_ID = 1487784315369689199
MOD_ID = 1461435337791373352

# A 3 SPECIFIKUS RANG ID-JA A VIDEÓHOZ (Ide írd az ID-kat!)
MIDDLEMAN_ID = 0  # {🥉} | Middle Man (100M)
SENIOR_MM_ID = 0  # {🥈} | Senior Middleman (250M)
ELITE_MM_ID = 0   # {🥇} | Elite middleman (315M+-)
...
async def on_message(msg):
    ...
    indok = None
    if re.search(LINK_REGEX, txt) or "tenor.com" in txt or "giphy.com" in txt: indok = "Link vagy GIF küldése"
...
async def mute(i, tag: discord.Member, időtartam: str, indok: str):
    if is_target_mod(tag): return await i.response.send_message("🛑 Moderátort nem némíthatsz le!", ephemeral=False)
    raw = időtartam.lower(); sec = int(raw.replace('m',''))*60 if 'm' in raw else int(raw)*60
    await tag.timeout(datetime.timedelta(seconds=sec))
...
async def video(i, szoveg: str, video: discord.Attachment):
    ...
    data["count"] = data.get("count", 9) + 1; save_json(VIDEO_FILE, data)
...
@app_commands.check(mod_and_up)
async def start_giveaway(i): await i.response.send_modal(GiveawayModal())
