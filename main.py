...
def is_mod(member: discord.Member):
    roles = ["Moderátor", "Admin", "Tulajdonos"]
...
async def on_message(message):
    ...
    if re.search(r"https?://", content):
        await message.delete()
        await mute_user(message.author, 600, "Linkek tiltottak")
...
async def warn(interaction: discord.Interaction, tag: discord.Member, indok: str):
    ...
    embed = discord.Embed(
        ...
        description=f"Tag: {tag.mention}\nIndok: **{indok}**\nFigyelmeztette: {interaction.user.mention}",
        color=0xf1c40f
    )
    embed.set_footer(text=FOOTER)
...
async def warns_cmd(interaction: discord.Interaction, tag: discord.Member):
    w = warns.get(tag.id, [])
    if not w:
        await interaction.response.send_message("Nincs figyelmeztetése")
        return
...
async def love(interaction: discord.Interaction, tag: discord.Member):
    p = random.randint(0, 100)
    await interaction.response.send_message(f"❤️ Szerelmi esély: **{p}%**")
